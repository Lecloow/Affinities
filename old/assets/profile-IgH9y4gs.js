import{S as g,A as m}from"./storage-m5hx6f6E.js";/* empty css                */class D{constructor(){this.contentEl=null,this.hintsData=null,this.userStats=null,this.candidates=null,this.refreshInterval=null,this.timerInterval=null,this.selectedDay=1,this.init(),window.addEventListener("beforeunload",()=>this.cleanup())}adjustServerTime(e){const s=new Date(e);return s.setHours(s.getHours()+1),s}cleanup(){this.refreshInterval!==null&&(window.clearInterval(this.refreshInterval),this.refreshInterval=null),this.timerInterval!==null&&(window.clearInterval(this.timerInterval),this.timerInterval=null)}init(){this.contentEl=document.getElementById("content"),new URLSearchParams(window.location.search).get("demo")==="1"&&m.login("demo").catch(()=>{}),this.render(),this.refreshInterval=window.setInterval(()=>{this.loadAndRenderHints()},3e4),this.timerInterval=window.setInterval(()=>{this.updateTimersOnly()},1e3)}async render(){if(console.log("🔥🔥🔥 PROFILE RENDER START"),!this.contentEl)return;const e=localStorage.getItem("saint_valentin_user");console.log("🔥🔥🔥 RAW STORAGE:",e);const s=g.getUser();if(console.log("🔥🔥🔥 USER:",s),!s){console.log("🔥🔥🔥 NO USER FOUND"),this.renderNotConnected();return}console.log("🔥🔥🔥 USER OK, RENDERING PROFILE"),await this.renderProfile(s)}renderNotConnected(){this.contentEl&&(this.contentEl.innerHTML=`
    ...
    `,await this.loadAndRenderHints()}async loadAndRenderHints(){const e=g.getUser();if(e)try{const[s,t,a]=await Promise.all([m.getHints(e.id),m.getUserStats(e.id).catch(i=>(console.warn("Failed to load user stats:",i),{user_id:e.id,total_points:0,code_exchange_bonus:0,guesses:[]})),m.getCandidates(e.id).catch(i=>(console.warn("Failed to load candidates:",i),{candidates:[]}))]);this.hintsData=s,this.userStats=t,this.candidates=a,this.renderHints()}catch(s){console.error("Error loading hints:",s);const t=document.getElementById("hints-section");t&&(t.innerHTML=`
  ...
  `}async handleRevealAllHints(e){const s=g.getUser();if(s)try{const t=document.querySelector(`.global-reveal-btn[data-day="${e}"]`);t&&(t.disabled=!0,t.textContent="Révélation en cours…");const a=await m.revealAllHints(s.id,e);await this.loadAndRenderHints(),a.revealed_count>0&&console.log(`${a.revealed_count} hint(s) revealed successfully`)}catch(t){console.error("Error revealing hints:",t),alert("Erreur lors de la révélation des indices. Veuillez réessayer."),await this.loadAndRenderHints()}}getTimeTagHtml(e,s){const t=new Date,a=e.getTime()-t.getTime();if(s.isActive){const r=t.getTime()-e.getTime(),o=Math.floor(r/6e4),l=Math.floor(o/60),d=o%60;let u;return l>0?u=`${l}h${d>0?` ${d}min`:""}`:u=`${o>0?o:1} min`,`<span class="time-tag"><span>Il y a</span><span>${u}</span></span>`}if(a<=0)return`<span class="time-tag">${s.nowLabel??"Maintenant !"}</span>`;const i=Math.floor(a%(1e3*60*60)/6e4),n=Math.floor(a%6e4/1e3);if(a<60*60*1e3){const r=i>0?`${i}min ${n}s`:`${n}s`;return`<span class="time-tag" data-timer data-target="${e.toISOString()}">
    ...
    `:""}renderGuessSection(e){var $,y,S,x;if(!g.getUser()||!this.candidates)return"";const t=new Date(e.reveal_time);if(new Date>=t)return`
      ...
      `;const i=(($=this.userStats)==null?void 0:$.guesses.filter(v=>v.day===e.day))||[],n=e.hints,c=((y=n[0])==null?void 0:y.revealed)||!1,r=((S=n[1])==null?void 0:S.revealed)||!1,o=((x=n[2])==null?void 0:x.revealed)||!1,l=i.some(v=>v.hint_number===1),d=i.some(v=>v.hint_number===2),u=i.some(v=>v.hint_number===3);let h=0;c&&!l?h=1:r&&!d?h=2:o&&!u&&(h=3);const p=[c,r,o].filter(Boolean).length;let f=0;p===1?f=100:p===2?f=75:p===3&&(f=50);let b="";return i.length>0&&(b='<div class="guess-history">',b+='<div class="guess-history-title">Tes tentatives:</div>',i.forEach(v=>{var E;const T=v.is_correct?"✓":"✗",H=v.is_correct?"success":"error",w=(E=this.candidates)==null?void 0:E.candidates.find(R=>R.id===v.guessed_user_id),_=w?`${w.first_name} ${w.last_name}`:"Inconnu";b+=`
    ...
    `}calculateGuessPoints(e){return{1:100,2:75,3:50}[e]||0}async handleSubmitGuess(e,s){var r,o;const t=g.getUser();if(!t)return;const a=(r=this.hintsData)==null?void 0:r.days.find(l=>l.day===e);if(!a){alert("Impossible de trouver les indices pour ce jour");return}const i=((o=this.userStats)==null?void 0:o.guesses.filter(l=>l.day===e))||[],n=a.hints.find((l,d)=>{const u=i.some(h=>h.hint_number===d+1);return l.revealed&&!u}),c=n?a.hints.indexOf(n)+1:0;if(c===0){alert("Aucun indice disponible pour deviner pour le moment !");return}try{const l=document.querySelector(".guess-form"),d=l==null?void 0:l.querySelector(".guess-submit-btn");d&&(d.disabled=!0,d.textContent="Envoi en cours...");const u=await m.submitGuess(t.id,e,c,s);await this.loadAndRenderHints(),u.is_correct?alert(`🎉 ${u.message}

Tu gagne ${u.points_earned} points!`):alert(`😔 ${u.message}`)}catch(l){console.error("Error submitting guess:",l),alert(l.message||"Erreur lors de l'envoi de ta réponse. Veuillez réessayer."),await this.loadAndRenderHints()}finally{const l=document.querySelector(".guess-form"),d=l==null?void 0:l.querySelector(".guess-submit-btn");d&&(d.disabled=!1,d.textContent="Valider mon choix")}}renderRevealCodeSection(e){if(!e.match_revealed)return"";const s=g.getUser();return s?(this.loadRevealCode(s.id,e.day),`<div id="reveal-code-container-${e.day}"></div>`):""}async loadRevealCode(e,s){try{const t=await m.getRevealCode(e,s),a=document.getElementById(`reveal-code-container-${s}`);if(!a)return;if(!t.available){a.innerHTML="";return}if(t.both_exchanged){a.innerHTML=`
    ...
    `;const n=a.querySelector(".code-exchange-form");n&&!t.exchanged&&n.addEventListener("submit",async c=>{c.preventDefault();const r=n.querySelector(".code-exchange-input");r&&r.value&&await this.handleExchangeCode(s,r.value)})}catch(t){console.error("Error loading reveal code:",t)}}async handleExchangeCode(e,s){const t=g.getUser();if(t)try{const a=document.querySelector(".code-exchange-form"),i=a==null?void 0:a.querySelector(".code-exchange-btn");i&&(i.disabled=!0,i.textContent="Échange en cours...");const n=await m.exchangeCode(t.id,e,s.toUpperCase());await this.loadAndRenderHints(),alert(`🎉 ${n.message}
